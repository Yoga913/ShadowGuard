...
# DNI / CIF
# Periksa di https://www.squobble.com/util/dni.html
def dnigen():
    number = ""
    number = number+random.choice("123456789")
    for i in range(7):
...
# Nomor kartu kredit
# Periksa di https://www.mobilefish.com/services/credit_card_number_checker/credit_card_number_checker.php
def ccgen():
    number = ""
...
# Nomor rekening bank
# Periksa di Check on https://bank.codes/iban/validate
def bankgen():
    number = ""
    while len(number) != 24:
        number = ""
...
# Keamanann Sosial
# Periksa di https://www.asinom.com/calculador-numero-seguridad-social.php
def ssgen():
    number = ""
    for i in range(2):
...
def number(args):
    delim = ","
    print("Nama, Nama Keluarga, Email, Tanggal Lahir, ID, Telepon Rumah, Ponsel, Kota, Jamsostek, Kartu Kredit, Masa Berlaku, CVV, Nomor Rekening (IBAN)")
...
#    print(colored(banner, "blue"))
    ...
    parser.add_argument("-n","--number", type=int, help="Jumlah rekaman yang harus dibuat")
    parser.add_argument("-b","--bankdata", action="store_true", help="Tampilkan hanya data bank (Card, CVV, IBAN..)")
    parser.add_argument("-e","--extended", action="store_true", help="Tampilkan hanya info tambahan (Kota, Telepon, SS..)")
    parser.add_argument("-f","--file", type=str, help="Jalur file untuk menyimpan data")
    parser.add_argument("-z","--zip", action="store_false", help="Kompres data ke file zip")
    parser.add_argument("-p","--password", type=str, help="Kata sandi untuk melindungi file zip")
    ...
    if args.number is not None:
        if args.file is None:
            ...
            print(colored("[+] Data berhasil dibuat!\n", "green")) 
    ...
    if args.file is not None:
        print(colored("[+] Data berhasil disimpan!\n", "green"))
